...
main_effect_long_demo<-main_effect_long %>% merge(full_dataset_aim3[,c(key_cols,confounder_cols)])
```
...
for (ot in outcome_types){
  ...
  outcome_table_3month<-main_effect_long_demo %>% filter(outcome_type==ot & outcome_time %in% c("baseline","1wkpost"))
...
#  outcome_table_3month$outcome_time_int <- as.integer(outcome_table_3month$outcome_time_f)
  ...
  library(lme4)
  model_base <- lme4::lmer(outcome_value~intervention_group+outcome_time_f + age365 + birthsex_factor + (1|SID),outcome_table_3month)
  model_int <- lme4::lmer(outcome_value~intervention_group*outcome_time_f + age365 + birthsex_factor + (1|SID),outcome_table_3month)
...
for (ot in outcome_types){
  ...
  outcome_table_3month<-main_effect_long_demo %>% filter(outcome_type==ot & outcome_time %in% c("baseline","1wkpost","3mopost"))
  
  outcome_table_3month$outcome_time_f <- factor(outcome_table_3month$outcome_time,ordered = FALSE,levels=c("baseline","1wkpost","3mopost"))
  outcome_table_3month$outcome_time_int <- as.integer(outcome_table_3month$outcome_time_f)
  
  
  library(lme4)
  model_base <- lme4::lmer(outcome_value~intervention_group+outcome_time_int + age365 + birthsex_factor + (1|SID),outcome_table_3month)
  model_int <- lme4::lmer(outcome_value~intervention_group*outcome_time_int + age365 + birthsex_factor + (1|SID),outcome_table_3month)
