...
for subject in $subject_list; do
    ...
    sbatch --export ALL,subid=${subid} \
           --job-name subject_corr \
		   --partition=ctn \
		   --cpus-per-task=2 \
		   --mem=8G \
		   --time=10:00:00 \
		   -o "${output_dir}"/"${subid}"_subject_corr_output.txt \
		   -e "${output_dir}"/"${subid}"_subject_corr_error.txt \
		   --account=sanlab \
		   /projects/sanlab/shared/DEV/DEV_scripts/rsfMRI/baseline_analysis/scripts/job_subject_level_corr.sh
done