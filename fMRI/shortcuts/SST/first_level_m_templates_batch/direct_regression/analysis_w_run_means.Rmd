...
```{r}
time_points <- readr::read_csv(paste0(dropbbox_dir,"/SST_roi_by_time_point_striatum.csv"))
```
...
df_offset_long$ROI<-stringr::str_extract_all(df_offset_long$ROI_fullname,pattern = "(?<=harvardoxford\\.subcortical_prob_)(.*)") %>% sub("\\."," ",.)
...
ggplot_time_points_extras <- list(
  ...
  labs(
    ...
    subtitle=paste0("Across ", as.character(subj_count)," subjects with 255 trials each"),
