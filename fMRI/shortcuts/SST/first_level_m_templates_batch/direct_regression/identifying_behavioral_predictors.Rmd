...
post_pre_rt_data <- sst_all_data%>% 
  ...
  select(trial_n,reaction_time,subid,waveid,runid,condition,next_last_rt_change)
```
...
FS_neural_behav_data_not_na<-FS_summary_data_accumbens[
  rowSums(is.na(FS_summary_data_accumbens[,c("response_amplitude","next_last_rt_change")]))==0 & #no NA values in correlation and
    is.finite(FS_summary_data_accumbens$response_amplitude) & is.finite(FS_summary_data_accumbens$next_last_rt_change) #no infinite values
    ,]
...
cor.test(FS_neural_behav_data_not_na$response_amplitude,FS_neural_behav_data_not_na$next_last_rt_change)
cor.test(FS_neural_behav_data_not_na$peak,FS_neural_behav_data_not_na$next_last_rt_change)
cor.test(FS_neural_behav_data_not_na$peak,FS_neural_behav_data_not_na$reaction_time)
cor_val<-cor.test(FS_neural_behav_data_not_na$trough,FS_neural_behav_data_not_na$next_last_rt_change)
print(cor_val)
```
...
FS_by_subj <- FS_by_subj_raw %>% filter(trial_count >2) %>%
  summarize(peak_cor_est = cor.test(peak,next_last_rt_change)$estimate,
            amplitude_cor_est = cor.test(response_amplitude,next_last_rt_change)$estimate,
            mean_next_last_rt_change=mean(next_last_rt_change),
            mean_peak=mean(peak),
...
```
```{r}
hist(FS_by_subj$mean_next_last_rt_change)
hist(FS_by_subj$mean_peak)
```


```{r}
cor.test(FS_by_subj$mean_next_last_rt_change,FS_by_subj$mean_peak)

cor.test(FS_by_subj$mean_next_last_rt_change,FS_by_subj$mean_amplitude)
```


```{r}
cor.test(FS_by_subj$mean_next_last_rt_change,FS_by_subj$mean_peak,method="spearman")
```
...
print(unique(FS_by_subj$ROI_delateralized))
...
for (roi_i in unique(FS_by_subj$ROI_delateralized)){
  ...
  print(cor.test(FS_by_subj_i$mean_next_last_rt_change,FS_by_subj_i$mean_peak))

  print(cor.test(FS_by_subj_i$mean_next_last_rt_change,FS_by_subj_i$mean_amplitude))
  print(cor.test(FS_by_subj_i$mean_next_last_rt_change,FS_by_subj_i$mean_peak,method="spearman"))
...
This result would withstand 10 multiple comparisons...not 50...so I'm not so sure this is a good place to start.
...
model_random_effects <- lme4::lmer(
   next_last_rt_change ~ trial_n + response_amplitude + (1+ response_amplitude | subid),
  FS_neural_behav_FOC
  )
...
model_fixed_effects <- lme4::lmer(
   next_last_rt_change ~ trial_n + response_amplitude + (1 | subid),
  FS_neural_behav_FOC
  )
...
model_random_effects2 <- lme4::lmer(
   next_last_rt_change ~ trial_n_r*response_amplitude + (1+ response_amplitude | subid),
  FS_neural_behav_FOC
  )
...
model_random_effects3_no_fixed <- lme4::lmer(
   next_last_rt_change ~ trial_n + (1+ response_amplitude | subid),
  FS_neural_behav_FOC
  )
...
```{r}
sst_all_data %>% select(trial_n,condition, reaction_time_clean, leading_rt, lagging_rt, next_last_rt_change, post_current_rt_change) %>% filter(condition!="Cue")
...
corrplot::corrplot(
  ...
  FS_neural_behav_data_not_na %>% select(
    ...
    post_current_rt_change, post_pre_rt_change, next_last_rt_change,
    #measures of RPE
...
### RPE_SSD based and next_last_rt_change
...
model_random_effects <- lme4::lmer(
   response_amplitude ~ next_last_rt_change+RPE_SSD_based +last_tone_delay+ (1+ next_last_rt_change | subid),
...
### considering trials since last SSD
...
model_random_effects <- lme4::lmer(
   response_amplitude ~ next_last_rt_change*trials_since_last_SSD+RPE_SSD_based+last_tone_delay+ (1+ next_last_rt_change | subid),
  FS_neural_behav_data_not_na
  )
summary(model_random_effects)
...
model_random_effects <- lme4::lmer(
   response_amplitude ~ next_last_rt_change*trials_since_last_SSD+RPE_SSD_based*trials_since_last_SSD+last_tone_delay +(1+ next_last_rt_change | subid),
  FS_neural_behav_data_not_na
  )
summary(model_random_effects)
...
```{r}
FS_neural_behav_data_not_na$LastTrialIsSSD<-FS_neural_behav_data_not_na$trials_since_last_SSD<2
model_random_effects <- lme4::lmer(
   response_amplitude ~ next_last_rt_change*trials_since_last_SSD+RPE_SSD_based*trials_since_last_SSD+last_tone_delay+LastTrialIsSSD +(1+ next_last_rt_change | subid),
  FS_neural_behav_data_not_na
  )
summary(model_random_effects)
```

This suggests learning the *likelihood* of an SSD trial; and it appears--if you _just_ had a SSD, you're likely to have a _higher_ response amplitude this time. This would be in the rare cases where subjects actually have two Stop trials in a row. This doesn't happen often.

And it's kind of remarkable that this wipes out _any_ other effect, including the next_last_rt_change. if the current trial is an SSD, nothing else matters...



### post_current_rt_change
...
model_fixed_effects <- lme4::lmer(
   response_amplitude ~ lagging_rt + leading_rt+next_last_rt_change+(1 + leading_rt| subid),
  FS_neural_behav_data_not_na
  )
summary(model_fixed_effects)
...
Of coures that's rank deficient, because `next_last_rt_change` is just hte difference of leading and lagging rt.
...
model_fixed_effects <- lme4::lmer(
   response_amplitude ~ leading_rt+lagging_rt+(1 + next_last_rt_change| subid),
  FS_neural_behav_data_not_na
  )
summary(model_fixed_effects)
...
### comparing early and late trials using trials_from_end
...
model_fixed_effects <- lme4::lmer(
   response_amplitude ~ leading_rt+lagging_rt+leading_rt:trials_from_end+lagging_rt:trials_from_end+(1 + next_last_rt_change| subid),
...
### post_current_rt effect is weaker than next_last_rt
