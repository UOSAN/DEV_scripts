...
waveNames = {'1', '2'};
writeDir = '~/Google Drive/oregon/preprocessing/DEV_scripts/fMRI/fx/multiconds/WTP/betaseries_ns_2s_at_fingerpress';
studyName = 'DEV';
filePattern = 'DEV.';
rowNum = -1;
            ...
            if exist(subFile)
                ...
                for k = 1:numel(runNames)
                    ...
                    if isfield(Data, (char(run))) && isfield(Data.(char(run)), 'FoodOnset')
                        ...
                        onsets = num2cell(cell2mat(Data.(char(run)).BidOnset)+cell2mat(Data.(char(run)).RT));
