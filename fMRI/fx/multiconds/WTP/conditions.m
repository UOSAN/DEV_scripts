...
condNames = {'healthy_liked','unhealthy_liked','healthy_disliked','unhealthy_disliked'};
                ...
                for k = 1:numel(runNames)
                    ...
                    if isfield(Data, (char(run))) && isfield(Data.(char(run)), 'FoodOnset')
                        ...
                        for a = 1:length(trial_onsets)
                            %identify which onset this condition this item
                            cond_i = find(strcmp(condNames,trial_names{a}));
                            %assign onset
