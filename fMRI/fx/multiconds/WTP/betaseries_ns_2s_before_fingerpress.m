...
waveNames = {'1', '2'};
writeDir = '~/Google Drive/oregon/preprocessing/DEV_scripts/fMRI/fx/multiconds/WTP/betaseries_ns_2s_before_fingerpress';
studyName = 'DEV';
filePattern = 'DEV.';
rowNum = -1;
            ...
            if exist(subFile)
                ...
                for k = 1:numel(runNames)
                    ...
                    if isfield(Data, (char(run))) && isfield(Data.(char(run)), 'FoodOnset')
                        ...
                        %disp(Data.(char(run)).BidOnset);
                        %disp(Data.(char(run)).RT);
                        % get time point just leading up to finger press
                        onset_offset = -2;
                        onsets = num2cell(onset_offset + cell2mat(Data.(char(run)).BidOnset)+cell2mat(Data.(char(run)).RT));
                        ...
                        durations = num2cell(repmat(2.0,1,16));
