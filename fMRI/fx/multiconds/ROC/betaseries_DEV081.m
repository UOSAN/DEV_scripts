...
for j = 1:numel(runNames)
    ...
    if ~isempty(subFileName)
        ...
        if j == 1 % second file is also R1, so use only first run
            subFile = sprintf('%s/%s', inputDir, subFileName{1}); % second file is also R1, so use only first run
        else
...
writetable(table,fullfile(writeDir, sprintf('trials_%s_%s.csv', sub, wave)),'Delimiter',',');
fprintf('\nTrial info saved in %s\n', fullfile(writeDir, sprintf('trials_%s_%s.csv', sub, wave)))

% save event info
writetable(eventtable,fullfile(writeDir, sprintf('events_%s_%s.csv', sub, wave)),'Delimiter',',')
fprintf('\nEvent info saved in %s\n', fullfile(writeDir, sprintf('events_%s_%s.csv', sub, wave)))