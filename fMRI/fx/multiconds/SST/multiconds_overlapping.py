...
def main(input_dir: str, bids_dir: str = None, file_limit=None,
    ...
    #now grab the jitter.txt
    #skip the first row and read everything from the second row as a tab-delimited file.
    #
    jitter_values = pd.read_table(Path(input_dir) / 'jitter.txt',skiprows=1,header=None,sep='\t').loc[:,0]
    ...
    for f in files:
        ...
        if match:
            ...
            # trial onsets and durations, response time points, tone time points, 
            masks_dict = create_event_conditions(all_data, jitter_values)
            ...
            if bids_dir:  # create MAT files storing behavioral information in bids format
                ...
                conditions = create_conditions(trial_start_time, trial_duration, list(posterror_masks_dict.values()), list(posterror_masks_dict.keys()))
    ...
    save_varying_condition_list(output_folder = output_folder,
                            ...
                            target_conditions = ['CorrectGo', 'CorrectStop', 'FailedGo', 'FailedStop'])
