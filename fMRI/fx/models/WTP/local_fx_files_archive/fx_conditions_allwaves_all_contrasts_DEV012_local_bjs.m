...
if ismatrix(included_runs_by_wave) && ~iscell(included_runs_by_wave)
...
for run_i = 1:length(included_runs_by_wave{1})
    ...
    %matlabbatch
    matlabbatch{run_i}.cfg_basicio.file_dir.file_ops.file_fplist.dir = {'/Users/benjaminsmith/Documents/oregon/data/DEV/bids_data/derivatives/fmriprep_2022/sub-DEV012/ses-wave1/func/'};
    matlabbatch{run_i}.cfg_basicio.file_dir.file_ops.file_fplist.filter = sprintf('s6_sub-DEV012_ses-wave1_task-WTP_acq-%d_space-MNI152NLin2009cAsym_desc-preproc_bold.nii',run_name);
...
for run_i = 1:length(included_runs_by_wave{1})
    run_name = included_runs_by_wave{1}(run_i);
    file_selector_string = sprintf('File Selector (Batch Mode): Selected Files (s6_sub-DEV012_ses-wave1_task-WTP_acq-%d_space-MNI152NLin2009cAsym_desc-preproc_bold.nii)',run_name);
...
for run_i = 1:length(included_runs_by_wave{2})
    ...
    matlabbatch{length(matlabbatch)+1}.cfg_basicio.file_dir.file_ops.file_fplist.dir = {'/Users/benjaminsmith/Documents/oregon/data/DEV/bids_data/derivatives/fmriprep_2022/sub-DEV012/ses-wave2/func'};
    matlabbatch{length(matlabbatch)}.cfg_basicio.file_dir.file_ops.file_fplist.filter = sprintf('s6_sub-DEV012_ses-wave2_task-WTP_acq-%d_space-MNI152NLin2009cAsym_desc-preproc_bold.nii',run_name);
...
for run_i = 1:length(included_runs_by_wave{2})
    ...
    file_selector_string = sprintf('File Selector (Batch Mode): Selected Files (s6_sub-DEV012_ses-wave2_task-WTP_acq-%d_space-MNI152NLin2009cAsym_desc-preproc_bold.nii)',run_name);
...
matlabbatch{length(matlabbatch)+1}.spm.stats.fmri_spec.dir = {'/Users/benjaminsmith/Documents/oregon/data/DEV/nonbids_data/fMRI/fx/models/WTP/all_waves/conditions/sub-DEV012'};
...
for wave_i = 1:length(included_runs_by_wave)
    for run_i = 1:length(included_runs_by_wave{wave_i})
        ...
        matlabbatch{length(matlabbatch)}.spm.stats.fmri_spec.sess(spec_i).multi = {sprintf('/Users/benjaminsmith/Documents/oregon/code/DEV_scripts/fMRI/fx/multiconds/WTP/conditions/DEV012_%d_WTP%d.mat',wave_i,run_name)};
        matlabbatch{length(matlabbatch)}.spm.stats.fmri_spec.sess(spec_i).regress = struct('name', {}, 'val', {});
        matlabbatch{length(matlabbatch)}.spm.stats.fmri_spec.sess(spec_i).multi_reg = {sprintf('/Users/benjaminsmith/Documents/oregon/data/DEV/bids_data/derivatives/auto-motion-fmriprep/sub-DEV012/sub-DEV012_ses-wave%d_task-WTP_acq-%d_run-1_desc-motion_regressors.txt',wave_i,run_name)};
