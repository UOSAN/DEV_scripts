...
CS_neural_behav_data_not_na<-FS_summary_data_accumbens[
...
```{r}
CS_by_subj_raw <- CS_neural_behav_data_not_na %>% group_by(subid) %>%
  mutate(trial_count=n())
CS_by_subj <- CS_by_subj_raw %>% filter(trial_count >2) %>%
...
t.test(CS_by_subj$peak_cor_est)
t.test(CS_by_subj$amplitude_cor_est)
...
```{r}
hist(CS_by_subj$peak_cor_est,breaks=40)
```
```{r}
hist(CS_by_subj$mean_post_pre_rt_change)
hist(CS_by_subj$mean_peak)
```


```{r}

cor.test(CS_by_subj$mean_post_pre_rt_change,CS_by_subj$mean_trough)

cor.test(CS_by_subj$mean_post_pre_rt_change,CS_by_subj$mean_peak)

cor.test(CS_by_subj$mean_post_pre_rt_change,CS_by_subj$mean_amplitude)
```


```{r}
cor.test(CS_by_subj$mean_post_pre_rt_change,CS_by_subj$mean_peak,method="spearman")
```
...
CS_neural_behav_data_not_na<-trial_neural_behav_data_not_na[trial_neural_behav_data_not_na$condition=="CorrectStop",]
```
...
CS_by_subj_raw <- CS_neural_behav_data_not_na %>% group_by(ROI_delateralized, subid) %>%
  mutate(trial_count=n())
CS_by_subj <- CS_by_subj_raw %>% group_by(ROI_delateralized, subid,trial_count) %>% filter(trial_count >2) %>%
  summarize(peak_cor_est = cor.test(peak,post_pre_rt_change)$estimate,
...
CS_by_subj<-merge(CS_by_subj,
                  full_run_data %>% filter(runid==1 & waveid==1),
                  by='subid',how='left'
                  )

CS_neural_behav_data_not_na <- merge(
  CS_neural_behav_data_not_na %>% filter(runid==1),
  full_run_data %>% filter(runid==1 & waveid==1),
...
for (roi_i in unique(CS_by_subj$ROI_delateralized)){
  # cat("\n")
  # cat(roi_i)
  # cat("\n")
  peak<-t.test(CS_by_subj %>% filter(ROI_delateralized==roi_i) %>% .$peak_cor_est)
  trough<-t.test(CS_by_subj %>% filter(ROI_delateralized==roi_i) %>% .$trough_cor_est)
  amplitude<-t.test(CS_by_subj %>% filter(ROI_delateralized==roi_i) %>% .$amplitude_cor_est)
  # print(peak)
  # print(trough)
  # print(amplitude)
...
for (roi_i in unique(CS_by_subj$ROI_delateralized)){
  ...
  cat(roi_i)
  cat("\n")
  CS_by_subj_i <- CS_by_subj %>% filter(ROI_delateralized==roi_i)
  
  peak<-cor.test(CS_by_subj_i$mean_post_pre_rt_change,CS_by_subj_i$mean_peak)
  trough<-cor.test(CS_by_subj_i$mean_post_pre_rt_change,CS_by_subj_i$mean_trough)
  amplitude<-cor.test(CS_by_subj_i$mean_post_pre_rt_change,CS_by_subj_i$mean_amplitude)
...
```{r}
#CS_neural_behav_data_not_na$trial_n_c<-128-CS_neural_behav_data_not_na$trial_n

library(lme4)

CS_neural_behav_p <- CS_neural_behav_data_not_na %>% filter(ROI_delateralized=="CueFollowing(CS>FS)striatal_cluster_combined")
...
CS_neural_behav_p$peak_z<-scale(CS_neural_behav_p$peak)
CS_neural_behav_p$trough_z<-scale(CS_neural_behav_p$trough)
CS_neural_behav_p$post_pre_rt_change_z<-scale(CS_neural_behav_p$post_pre_rt_change)
CS_neural_behav_p$P_stop_trial_change_z<-scale(CS_neural_behav_p$P_stop_trial_change)
CS_neural_behav_p$P_stop_trial_z<-scale(CS_neural_behav_p$P_stop_trial)
...
P_stop_trial_over_trials<-CS_neural_behav_p %>% group_by(trial_n) %>% summarize(
...
model_full_effects <- lme4::lmer(
   peak ~ trial_n_n +post_pre_rt_change + P_stop_trial_post +(1+ post_pre_rt_change | subid),
  CS_neural_behav_p %>% filter(trial_n>50)
  )
summary(model_full_effects)
...
```{r}
CS_neural_behav_p$P_stop_trial_post_z<-scale(CS_neural_behav_p$P_stop_trial_post)
...
```{r}
fs_by_ppt<-merge(data_by_ppt, CS_by_subj_i,data,by.x = "SID",by.y="subid")
