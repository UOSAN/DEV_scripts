...
{
  time_points_raw <- readr::read_csv(paste0(dropbbox_dir,"/SST_roi_by_time_point.csv")) %>% filter(wave %in% c(1,2))
  #now only include subjects that have been deemed includeable
  #this is the set that made up the ROI.
  included_subjects_table <-readr::read_csv(paste0(config::get("level2_output_path"),
                                    "conditions_20230616/raw_filelist.csv"))
...
if(!loaded_from_cache){
  sst_all_data_accuracy <- sst_all_data %>% select(-numchunks_reciprocal,-ladder_movement,-rundatetime,-RPE_SSD_based,-RPE_SSD_RT_diff,-ssrt_0)
  ...
  sst_all_data <- sst_all_data %>% filter(
    stop_prop_correct>prop_correct_included_range[[1]] & 
      stop_prop_correct<prop_correct_included_range[[2]])
  print("after removing very high and low performance trials:")
  print(sst_all_data$subid %>% unique %>% length)
  readr::write_csv(sst_all_data,file=paste0(dropbox_file_dir,"sst_behavioral_data_all_with_analysis_w1.csv"))
...
sst_all_data_accuracy_with_neural <- merge(trial_summary_data_delat,sst_all_data_accuracy,by.x = c("subid","wave","trial_n","condition"),by.y=c("subid","waveid","trial_n","condition"),all.x = TRUE)

included_subjects <- unique(sst_all_data_accuracy_with_neural$subid)
...
sst_accuracy_by_subject <- sst_accuracy_by_run %>% filter(waveid %in% c(1,2)) %>%
  filter(subid %in% included_subjects) %>% group_by(subid) %>% summarise(
  AnyRunRemoved=any(!included),
  AnyRunRetained=any(included),
  AllRunsRetained=all(included)
  )

removed_runs <- sst_accuracy_by_run[sst_accuracy_by_run$included==FALSE,]

  
print("removed runs:")
print(removed_runs)

print(table(removed_runs$stop_prop_correct_too_low))
print(table(removed_runs$stop_prop_correct_too_high))
...
removal_summary_stats<-sst_accuracy_by_run %>% ungroup %>% group_by(subid) %>% summarise(
  AnyRunRemoved=any(!included),
  AnyRunRetained=any(included),
  AllRunsRetained=all(included)
  )
...
print(table(removal_summary_stats$AnyRunRetained))

#removal_summary_stats[removal_summary_stats$AnyRunRetained,"subid"]
subjects_removed_entirely<-removal_summary_stats[!removal_summary_stats$AnyRunRetained,"subid"] %>% unlist()

#sst_accuracy_by_run[sst_accuracy_by_run$subid %in% subjects_removed_entirely,]
...
## Check if healthy correlates with stopping

A reviewer asked how we justify that activity associated with CS isn't a response to the food cue.

One justification would be if P(Stop | Signal) isn't higher for healthy vs. unhealthy food cues.

A more thorough justification would be if there wasn't any kind of distinction between the food cues for Stopping vs not stopping.

```{r}
table(sst_all_data$health)
colnames(sst_all_data)
#basic: health label doesn't predict failed Stop
sst_all_data %>% filter(go_no_go_condition_label=="Stop") %>%
  select(health_condition_label, go_no_go_condition_label,condition)

table(sst_all_data$health_condition_label,sst_all_data$health_condition)
sst_all_data$IsCorrect<-sst_all_data$condition %in% c("CorrectGo","CorrectStop")
summary(glm( IsCorrect~health_condition_label, family=binomial(link = "logit"),
     sst_all_data %>% filter(go_no_go_condition_label=="Stop")))

#OK, so there _is_ a relationship. So we should next look to see whether that striatal signal measured is associated with health vs. unhealth

model_full_effects <- lme4::lmer(
   med_post_trial_z ~ trial_n_s+ condition + P_stop_trial_change_z+ post_pre_rt_change_z + (1 +post_pre_rt_change_z  + P_stop_trial_change_z | subid)  + (1 | wave),
  trial_neural_behav_roi %>% filter(trial_n>50 & condition %in% c("CorrectStop","FailedStop"))
  )
summary(model_full_effects)


model_full_effects_w_health <- lme4::lmer(
   med_post_trial_z ~ trial_n_s+ condition + P_stop_trial_change_z+ post_pre_rt_change_z + health_condition_label + (1 +post_pre_rt_change_z  + P_stop_trial_change_z + health_condition_label | subid)  + (1 | wave),
  trial_neural_behav_roi %>% filter(trial_n>50 & condition %in% c("CorrectStop","FailedStop"))
  )
summary(model_full_effects_w_health)

anova(model_full_effects,model_full_effects_w_health)



```

## Check how many times there are stop trials following stop trials

```{r}

sst_all_active_data<-sst_all_data %>% arrange(subid, waveid, runid, trial_n) %>% 
  filter(go_no_go_condition_label!="NullTrial") %>%
  mutate(preceding_condition_label = lag(condition,1)) %>%
  mutate(FollowsStopTrial = preceding_condition_label %in% c("FailedStop","CorrectStop"))

sst_all_active_data %>% select(
  trial_n,condition,preceding_condition_label,FollowsStopTrial)

summary_counts_of_trial_types <- sst_all_active_data %>%
  arrange(subid, waveid, runid, trial_n) %>% 
  group_by(subid, waveid, runid) %>%
  summarise(CorrectGoCount=sum(condition=="CorrectGo"),
            FailedGoCount=sum(condition=="FailedGo"),
            CorrectStopCount=sum(condition=="CorrectStop"),
            FailedStopCount=sum(condition=="FailedStop")
            
            ) %>% ungroup(subid, waveid, runid) %>%
  select(-subid, -waveid,-runid) %>% colMeans()
summary_counts_of_trial_types
sum(summary_counts_of_trial_types)


summary_counts_of_following_trial_types<-sst_all_active_data %>% 
  arrange(subid, waveid, runid, trial_n) %>% 
  filter(FollowsStopTrial==TRUE) %>%
  group_by(subid, waveid, runid) %>%
  summarise(CorrectGoCount=sum(condition=="CorrectGo"),
            FailedGoCount=sum(condition=="FailedGo"),
            CorrectStopCount=sum(condition=="CorrectStop"),
...
cg_count<-summary_counts_of_following_trial_types[names(summary_counts_of_following_trial_types)=="CorrectGoCount"]

cg_count/sum(summary_counts_of_following_trial_types)
...
model_no_rt<- lme4::lmer(
   med_post_trial_z ~ trial_n_s+ condition + P_stop_trial_change_z + (1 +post_pre_rt_change_z  + P_stop_trial_change_z  | subid) + (1 | wave),
  trial_neural_behav_roi %>% filter(trial_n>50 & condition %in% c("CorrectStop","FailedStop"))
  )

anova(model_full_effects,model_no_rt)
model_no_p_stop<- lme4::lmer(
   med_post_trial_z ~ trial_n_s+ condition + post_pre_rt_change_z + (1 +post_pre_rt_change_z  + P_stop_trial_change_z  | subid) + (1|wave),
  trial_neural_behav_roi %>% filter(trial_n>50 & condition %in% c("CorrectStop","FailedStop"))
  )
...
model_list <- list("RT Change Only, Excluding P(Stop) Change"=model_no_p_stop,
                   "P(Stop) Change Only, Excluding RT Change"=model_no_rt,
...
```{r}
#save(model_list, file="temp_state.Rdata")
...
The below code block is designed to do some testing.
```{r}
#debugSource("~/Google Drive/oregon/code/DEV_scripts/fMRI/fx/models/SST/direct_regression/display_utils.R")
#load("temp_state.Rdata")
# table_cells <- create_sst_mlm_table(
#   model_list,name_transform =friendly_name_transform_table,do_model_comparison=TRUE)

```

```{r }
gtsave(table_cells,filename = "sst_paper_generation_w12_output/table1.html")
```
...
Are we able to _predict_ RPE? We've already demonstrated 

```{r}
relevant_data<- trial_neural_behav_roi %>% filter(trial_n>50 & condition %in% c("CorrectStop","FailedStop"))
predicted_overall_rpe <- predict(model_full_effects,relevant_data)
relevant_data$predicted_med_post_trial_z<-predicted_overall_rpe

ggplot(relevant_data,aes(x=med_post_trial_z,y=predicted_med_post_trial_z,color=condition))+geom_point(alpha=0.05)

cor.test(relevant_data$med_post_trial_z,relevant_data$predicted_med_post_trial_z)
```

```{r}
r.squaredGLMM(model_full_effects) 
```

## Reviewer request: Examine interaction

```{r}

model_main_effects <- lme4::lmer(
   med_post_trial_z ~ trial_n_s+ condition + P_stop_trial_change_z+ post_pre_rt_change_z + (1 +post_pre_rt_change_z  + P_stop_trial_change_z  | subid)  + (1 | wave),
  trial_neural_behav_roi %>% filter(trial_n>50 & condition %in% c("CorrectStop","FailedStop"))
  )
summary(model_main_effects)

model_interaction_effects <- lme4::lmer(
   med_post_trial_z ~ trial_n_s+ condition + P_stop_trial_change_z*post_pre_rt_change_z + (1 +post_pre_rt_change_z*P_stop_trial_change_z  | subid)  + (1 | wave),
  trial_neural_behav_roi %>% filter(trial_n>50 & condition %in% c("CorrectStop","FailedStop"))
  )
summary(model_interaction_effects)

anova(model_interaction_effects,model_main_effects)
```
...
is_not_na<-(
  !is.na(trial_neural_behav_roi$med_post_trial_z) & 
    !is.na(trial_neural_behav_roi$trial_n_z) &
    !is.na(trial_neural_behav_roi$post_pre_rt_change_z) &
    !is.na(trial_neural_behav_roi$P_stop_trial_change_z) &
    !is.na(trial_neural_behav_roi$subid) & 
    !is.na(trial_neural_behav_roi$wave))


    
relevant_data <- trial_neural_behav_roi %>% filter(trial_n>50 & condition %in% c("FailedStop") & is_not_na)
model_P_stop_trial_w_change <- lme4::lmer(
   med_post_trial_z ~ trial_n_z + post_pre_rt_change_z + P_stop_trial_change_z +(1+ post_pre_rt_change_z + P_stop_trial_change_z  | subid) + (1 | wave),
  relevant_data
  )
...
model_P_stop_trial_w_change_no_rt <- lme4::lmer(
   med_post_trial_z ~ trial_n_z  + P_stop_trial_change_z +(1+ post_pre_rt_change_z + P_stop_trial_change_z  | subid) + (1 | wave),
  relevant_data
  )
...
model_P_stop_trial_w_change_no_p_stop_d <- lme4::lmer(
   med_post_trial_z ~ trial_n_z +  post_pre_rt_change_z +(1+ post_pre_rt_change_z + P_stop_trial_change_z | subid) + (1 | wave),
  relevant_data
  )
...
model_list <- list("RT Change Only, Excluding P(Stop) Change"=model_P_stop_trial_w_change_no_p_stop_d,
                   "P(Stop) Change Only, Excluding RT Change"=model_P_stop_trial_w_change_no_rt,
...
```
```{r }
gtsave(table_cells,filename = "sst_paper_generation_w12_output/table2.html")
```
...
```{r}
sjplot_result <- sjPlot::tab_model(model_P_stop_trial_w_change)
```
...
#### R^2 variance explained estimate
...
r.squaredGLMM(model_P_stop_trial_w_change) 

relevant_data <- trial_neural_behav_roi %>% filter(trial_n>50 & condition %in% c("FailedStop") & is_not_na)

relevant_data$predicted_med_post_trial_z <- predict(model_P_stop_trial_w_change,relevant_data)

ggplot(relevant_data,aes(x=med_post_trial_z,y=predicted_med_post_trial_z,color=condition))+geom_point(alpha=0.2)
...
model_P_stop_trial_w_change <- lme4::lmer(
   med_post_trial_1_5 ~ trial_n_z + post_pre_rt_change_z + P_stop_trial_change_z +(1+ post_pre_rt_change_z + P_stop_trial_change_z  | subid) + (1 | wave),
...
#ignoring post_pre_rt_change_z
model_P_stop_trial_w_change_no_rt <- lme4::lmer(
   med_post_trial_1_5 ~ trial_n_z  + P_stop_trial_change_z +(1+ post_pre_rt_change_z + P_stop_trial_change_z  | subid) + (1 | wave),
  trial_neural_behav_roi %>% filter(trial_n>50 & condition %in% c("FailedStop"))
  )
#summary(model_P_stop_trial_w_change_no_rt)
...
#ignoring P_stop_trial_change_z
model_P_stop_trial_w_change_no_p_stop_d <- lme4::lmer(
   med_post_trial_1_5 ~ trial_n_z +  post_pre_rt_change_z +(1+ post_pre_rt_change_z + P_stop_trial_change_z | subid) + (1 | wave),
  trial_neural_behav_roi %>% filter(trial_n>50 & condition %in% c("FailedStop"))
  )
#summary(model_P_stop_trial_w_change_no_p_stop_d)
...
```{r }
gtsave(table_cells,filename = "sst_paper_generation_w12_output/table3.html")
```
...
behavioral_data_w_tone_data <- readr::read_csv(paste0(dropbbox_dir,"/sst_behavioral_data_all_w_class_type_reveal_onset.csv")) %>% filter(waveid %in% c(1,2))
...
ggplot_time_points_extras <- list(
  ...
  labs(
    ...
    subtitle=paste0("Across ", as.character(subj_count)," subjects with two sessions of 128 trials each"),
...
#ggsave(paste0(output_folder, "/figure_1.svg"))
...
figure_2 <- ggplot(trial_tps_compare_conditions,
...
ggsave(paste0(output_folder, "/figure_2.svg"),width = 6,height=4)

ggsave(paste0(output_folder, "/figure_2.png"),width = 6,height=4)
...
ggsave(paste0(output_folder, "/figure_3.svg"),width = 6,height=4)

ggsave(paste0(output_folder, "/figure_3.png"),width = 6,height=4)
...
df_offset_long$ROI_bilat <- gsub("Cingulate Gyrus, anterior division","ACC",df_offset_long$ROI_bilat)

next_trial_gradient$hrf_tp<-next_trial_gradient$tp+5

figure_4 <- ggplot(df_offset_long %>% filter(condition %in% c("FailedStop","CorrectStop")),
...
ggsave(paste0(output_folder, "/figure_4.svg"),width = 6,height=6)

ggsave(paste0(output_folder, "/figure_4.png"),width = 6,height=6)
...
labels$ROI_bilat <- gsub("Cingulate\nGyrus,\nanterior\ndivision", "ACC",labels$ROI_bilat)


labels <- labels %>% arrange(ROI_labels)
#these need to be in alphabetical order
labels$xpos <- c(6,10,13,10,11) 
labels$ypos <- c(-0.07,0.2,0.1,-0.07,0.15)

trial_tps_compare_conditions$ROI_bilat <- gsub("Cingulate Gyrus, anterior division","ACC",trial_tps_compare_conditions$ROI_bilat)

figure_5 <- ggplot(trial_tps_compare_conditions, aes(x=offset_start, y=CS_minus_FS,color=ROI_bilat))+
  geom_tile(data = next_trial_gradient, aes(x=hrf_tp,alpha=pct_in_range,y=0),fill="#77ff77",color="transparent")+
...
ggsave(paste0(output_folder, "/figure_5.svg"),width = 6,height=4)

ggsave(paste0(output_folder, "/figure_5.png"),width = 6,height=4)
