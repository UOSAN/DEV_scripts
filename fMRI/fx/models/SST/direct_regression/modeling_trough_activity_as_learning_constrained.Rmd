...
```
We've changed this code to use `P_stop_trial_change` ratehr than `P_stop_trial_post`. `P_stop_trial_change` makes more theoretical sense (since we're trying to measure how much the striatal effect is an update) yet it isn't a significant regressor.

```{r}
model_full_effects <- lme4::lmer(
   peak ~ trial_n_s+ condition + +P_stop_trial_change+P_stop_trial_post+ post_pre_rt_change +  (1 +post_pre_rt_change +  P_stop_trial_change +P_stop_trial_post  | subid),
  trial_neural_behav_roi %>% filter(trial_n>50)
  )
summary(model_full_effects)
