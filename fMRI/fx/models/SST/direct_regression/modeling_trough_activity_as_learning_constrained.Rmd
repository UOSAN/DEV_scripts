...
## otehr ways to measure responsiveness

Perhaps there are other ways to measure responsiveness to the probability of a stop trial. For instance, wtihin subject, we can measure the relationship of peak activity to the change in informativeness probability of a stop trial. Then, maybe across subjects, the strength of that relationship correlates with overall performance in the task.
...
peak_p_stop_change_cor <- FS_neural_behav_p %>% 
  group_by(subid,wave,runid) %>%
  filter((sum(((is.na(P_stop_trial_change) | is.na(P_stop_trial_change))==FALSE))>2)) %>%
  summarise(cor_P_stop_trial_change_peak=
              case_when(
                (sum(((is.na(P_stop_trial_change) | is.na(P_stop_trial_change))==FALSE))>2)~cor.test(peak,P_stop_trial_change)$estimate, #~cor.test(P_stop_trial_change,peak,na.rm=TRUE)$estimate,
            TRUE~ as.numeric(NA),
            last_ladder_delay
              )
  )
...
```{r}
FS_neural_behav_p %>% arrange(subid,wave,runid,trial_n) %>% 
  select(subid,wave,runid,trial_n,LadderX_SSD_ms)
...
model_P_stop_trial_w_change <- lme4::lmer(
   peak_z ~ trial_n_z +condition + post_pre_rt_change_z + P_stop_trial_change_z +(1+ post_pre_rt_change_z + P_stop_trial_change_z  | subid),
  trial_neural_behav_roi %>% filter(trial_n>50) %>% filter(condition %in% c("FailedStop","CorrectStop"))
  )
summary(model_P_stop_trial_w_change)
...
```{r}
model_P_stop_trial_w_change <- lme4::lmer(
   peak_z ~ trial_n_z  + post_pre_rt_change_z + P_stop_trial_change_z +(1+ post_pre_rt_change_z + P_stop_trial_change_z  | subid),
  trial_neural_behav_roi %>% filter(trial_n>50) %>% filter(condition %in% c("CorrectStop"))
  )
summary(model_P_stop_trial_w_change)

model_P_stop_trial_w_change_no_rt <- lme4::lmer(
   peak_z ~ trial_n_z  +  P_stop_trial_change_z + P_stop_trial_change_z +(1+ post_pre_rt_change_z + P_stop_trial_change_z  | subid),
  trial_neural_behav_roi %>% filter(trial_n>50) %>% filter(condition %in% c("CorrectStop"))
  )
summary(model_P_stop_trial_w_change_no_rt)
...
model_P_stop_trial_w_change_no_p_stop_d <- lme4::lmer(
   peak_z ~ trial_n_z  +  post_pre_rt_change_z +(1+ post_pre_rt_change_z + P_stop_trial_change_z | subid),
  trial_neural_behav_roi %>% filter(trial_n>50) %>% filter(condition %in% c("CorrectStop"))
  )
summary(model_P_stop_trial_w_change_no_p_stop_d)
...
model_P_stop_trial_w_change <- lme4::lmer(
   peak_z ~ trial_n_z + post_pre_rt_change_z + P_stop_trial_change_z +(1+ post_pre_rt_change_z + P_stop_trial_change_z  | subid),
  trial_neural_behav_roi %>% filter(trial_n>50) %>% filter(condition %in% c("FailedStop"))
  )
summary(model_P_stop_trial_w_change)

model_P_stop_trial_w_change_no_rt <- lme4::lmer(
   peak_z ~ trial_n_z + P_stop_trial_change_z +(1+ post_pre_rt_change_z + P_stop_trial_change_z  | subid),
  trial_neural_behav_roi %>% filter(trial_n>50) %>% filter(condition %in% c("FailedStop"))
  )
summary(model_P_stop_trial_w_change_no_rt)
...
Why is post_pre_rt_change_z so weak, after showing such strong results above?

```{r}

model_incremental <- lme4::lmer(
   peak_z ~ trial_n_z + post_pre_rt_change_z +(1+ post_pre_rt_change_z  | subid),
  trial_neural_behav_roi %>% filter(trial_n>50) %>% filter(condition %in% c("FailedStop"))
  )
summary(model_incremental)
```

Not really the inclusion of p_stop_trial
...
ahhh---it's because a lot of the work happens in the first few trials. So I should figure out whether eliminating the first 50 trials is something we _ought_ to do or not.
