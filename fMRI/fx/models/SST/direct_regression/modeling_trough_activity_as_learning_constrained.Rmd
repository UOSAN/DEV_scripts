...
## includ both CS and FS in the model
...
We have doen both of these before, right???
...
model_full_effects <- lme4::lmer(
   peak ~ trial_n_n +post_pre_rt_change + P_stop_trial + trials_since_last_SSD +(1+ post_pre_rt_change | subid),
  FS_neural_behav_p
  )
summary(model_full_effects)
...
model_full_effects <- lme4::lmer(
   peak ~ trial_n_n +post_pre_rt_change + P_stop_trial +(1+ post_pre_rt_change | subid),
  FS_neural_behav_p
  )
summary(model_full_effects)
...
model_full_effects <- lme4::lmer(
   peak ~ trial_n_n +post_pre_rt_change + P_stop_trial +(1+ post_pre_rt_change+P_stop_trial | subid),
  FS_neural_behav_p %>% filter(trial_n>50)
  )
summary(model_full_effects)

model_random_effects <- lme4::lmer(
   peak ~ trial_n_n +post_pre_rt_change  +(1+ post_pre_rt_change+P_stop_trial | subid),
  FS_neural_behav_p %>% filter(trial_n>50)
  )
...
model_full_effects <- lme4::lmer(
   peak ~ trial_n_n +post_pre_rt_change + P_stop_trial+I(P_stop_trial^2) +(1+ post_pre_rt_change+P_stop_trial | subid),
  FS_neural_behav_p %>% filter(trial_n>50)
  )
summary(model_full_effects)
...
Very impressive! It's the P_stop_trial that makes the difference..
