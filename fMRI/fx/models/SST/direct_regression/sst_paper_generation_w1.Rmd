...
```{r}
cache_filepath <- paste0(dropbbox_dir,"sst_paper_generation_w1.Rdata")
...
{
  time_points_raw <- readr::read_csv(paste0(dropbbox_dir,"/SST_roi_by_time_point.csv")) %>% filter(wave==1)
...
if(!loaded_from_cache){
  ...
  sst_all_data_raw <- readr::read_csv(sst_all_data_filepath)  %>% filter(waveid==1)
...
```{r }
gtsave(table_cells,filename = "sst_paper_generation_w1_output/table1.html")
```
...
```
```{r }
gtsave(table_cells,filename = "sst_paper_generation_w1_output/table2.html")
```
...
```{r }
gtsave(table_cells,filename = "sst_paper_generation_w1_output/table3.html")
```
...
behavioral_data_w_tone_data <- readr::read_csv(paste0(dropbbox_dir,"/sst_behavioral_data_all_w_class_type_reveal_onset.csv")) %>% filter(waveid==1)
