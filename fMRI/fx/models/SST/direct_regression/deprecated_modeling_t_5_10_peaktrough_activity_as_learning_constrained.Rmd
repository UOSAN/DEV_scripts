...
activity_by_subj <- trial_neural_behav_data_not_na %>% filter(condition %in% c("CorrectStop","FailedStop")) %>%
 group_by(subid,wave,runid,ROI_delateralized,condition) %>%
  ...
  tidyr::pivot_wider(names_from="condition", values_from=c("mean_med_post_stop_signal","mean_post_pre_rt")) %>%
  mutate(
...
by_subj <- merge(behav_by_subj,activity_by_subj, byx=c("subid","wavid","runid","subid","waveid","runid"))
by_subj_striatum<-by_subj %>% filter(ROI_delateralized=="CueFollowing(CS>FS)striatal_cluster_combined")


cor.test(by_subj_striatum$mean_ssrt_0,by_subj_striatum$post_stop_signal_CSMinusFS) # a good start


by_subj_striatum_no_outliers <- by_subj_striatum %>% filter(mean_ssrt_0>-0.2)
...
```{r}
by_subj_striatum_no_outliers <- by_subj_striatum %>% filter(mean_ssrt_0>-0.2 & stop_prop_correct>=0.4 & stop_prop_correct<=0.6)
