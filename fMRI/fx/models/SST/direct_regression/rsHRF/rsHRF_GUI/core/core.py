...
warnings.warn("bids module disabled")
...
class Core():
    ...
    def makeInput(self, inp):
        ...
        # if the input is present in a directory organized in BIDS format
            ...
            layout = BIDSLayout(input_file, validate=False,
            ...
            all_inputs = layout.get(return_type='filename', datatype='func', subject=subjects_to_analyze,
                                    task='rest', desc='preproc', suffix='bold', extension=['nii', 'nii.gz'])
            # if no *bold.nii or *bold.nii.gz files were found
            if not all_inputs != []:
                return Status(False, error='There are no files of type *bold.nii / *bold.nii.gz '
            ...
            if mode == "bids w/ atlas":
                for file_count in range(len(all_inputs)):
                    ...
                    temp = (self.makeInputFile(
                        all_inputs[file_count], mask_file, ".nii", 'bids w/ atlas'))
            ...
            # if the atlas file is not provided
                ...
                # obtaining the masks
                all_masks = layout.get(return_type='filename', datatype='func', subject=subjects_to_analyze,
                                       task='rest', desc='brain', suffix='mask', extension=['nii', 'nii.gz'])
                # if no mask files were found
                if not all_masks != []:
                    return Status(False, error='There are no files of type *mask.nii / *mask.nii.gz '
                                  'Please make sure to have at least one file of the above type '
                                  'in the BIDS specification')
                # if the number of mask files != number of input files
                if len(all_inputs) != len(all_masks):
                    return Status(False, error='The number of *bold.nii / .nii.gz and the number of '
                ...
                if not all_prefix_match:
                    ...
                    for file_count in range(len(all_inputs)):
                        temp = (self.makeInputFile(
                            all_inputs[file_count], all_masks[file_count], ".nii", 'bids'))
                        if not temp.get_state():
                            return temp
                    # returns the status
                    return Status(True, info="Preprocessed all input", dic={"Number of Input Subjects: ": len(all_inputs)})
    ...
    def makeInputFile(self, input_file, mask_file, file_type, mode):
        ...
        if not subject.is_present("BOLD", input_file):
            if file_type == ".nii" or file_type == ".nii.gz":
                ...
                _, nobs = data.shape
                data1 = np.reshape(data, (-1, nobs), order='F').T
            bold_ts = BOLD_Raw(label="BOLD", ts=np.array(
                data1), para=self.parameters, subject_index=subject.get_subject_index())
            # the Raw BOLD Time-series is associated to a particular input-file
            bold_ts.set_inputfile(input_file)
        ...
        if not subject.is_present("Preprocessed-BOLD", (self.parameters, mask_file, bold_ts)):
            ...
            bold_pre_ts = BOLD_Preprocessed(label="Preprocessed-BOLD", ts=np.array(
                bold_sig), para=self.parameters, subject_index=subject.get_subject_index())
            # the Preprocessed BOLD Time-series is associated to a particular Raw BOLD Time-Series
            bold_pre_ts.set_BOLD_Raw(bold_ts)
            bold_pre_ts.set_maskfile(mask_file)  # and a particular mask_file
            # adding the time-series objects
            subject.add_BOLD_pre(bold_pre_ts)
    ...
    def retrieveHRF(self, bold_pre_ts, get_pos=False):
        ...
        as the parameters
        """
        subject_index = bold_pre_ts.get_subject_index(
        )                    # gets the subject-index associated to the preprocessed BOLD Time-series
        subject = self.dataStore.get_subject_by_index(
            subject_index)  # gets the subject from the index
        ...
        if not subject.is_present("HRF", (self.parameters, bold_pre_ts)):
            ...
            bold_sig = processing. \
                rest_filter. \
                rest_IdealFilter(bold_sig, self.parameters.get_TR(
            ...
            if not (para['estimation'] == 'sFIR' or para['estimation'] == 'FIR'):
                ...
                bf = basis_functions.basis_functions.get_basis_function(
                    bold_sig.shape, para)       # obtaining the basis set
                ...
                hrfa = beta_hrf
            # instantiating the time-series objects
            hrf = HRF(label="HRF", ts=hrfa,
                      subject_index=subject_index, para=self.parameters)
    ...
    def getHRFParameters(self, hrf):
        ...
        for voxel_id in range(nvar):
            hrf1 = hrfa[:, voxel_id]
            PARA[:, voxel_id] = parameters.wgr_get_parameters(
                hrf1, para['TR'] / para['T'])
            for i in range(3):
    ...
    def deconvolveHRF(self, hrf):
        ...
        if not subject.is_present("Deconvolved-BOLD", (self.parameters, hrf)):
            ...
            for voxel_id in range(nvar):
                ...
                H = np.fft.fft(np.append(hrf_, np.zeros(
                    (nobs - max(hrf_.shape), 1))), axis=0)
                M = np.fft.fft(bold_sig[:, voxel_id])
                data_deconv[:, voxel_id] = np.fft.ifft(
                    H.conj() * M / (H * H.conj() + .1*np.mean((H * H.conj()))))
                event_number[:, voxel_id] = np.amax(event_bold[voxel_id].shape)
            # instantiating the time-series object
            dd = Bold_Deconv(label="Deconvolved-BOLD", ts=data_deconv,
                             subject_index=subject_index, para=hrf.get_parameters())
            # the deconvoled BOLD Time-series is associated to a particular HRF
            dd.set_HRF(hrf)
            dd.set_event_num(event_number)  # setting the BOLD events
            # adding the time-series to the subject
            pos = subject.add_BOLD_deconv(dd)
            # returning the status
