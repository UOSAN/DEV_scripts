...
for (roi_i in unique(FS_by_subj$ROI_delateralized)){
  ...
  print(t.test(FS_by_subj %>% filter(ROI_delateralized==roi_i) %>% .$trough_cor_est))
...
for (roi_i in unique(FS_by_subj$ROI_delateralized)){
  ...
  roi_inddiv_row<-data.frame("roi"=roi_i,"measure"="peak","value"=peak[[1]],"pvalue"=peak$p.value)
  roi_inddiv_list<-append(roi_inddiv_list,list(roi_inddiv_row))
  roi_inddiv_row<-data.frame("roi"=roi_i,"measure"="trough","value"=trough[[1]],"pvalue"=trough$p.value)
  roi_inddiv_list<-append(roi_inddiv_list,list(roi_inddiv_row))
  roi_inddiv_row<-data.frame("roi"=roi_i,"measure"="amplitude","value"=amplitude[[1]],"pvalue"=amplitude$p.value)
  roi_inddiv_list<-append(roi_inddiv_list,list(roi_inddiv_row))
  print(peak)
  print(trough)
  print(amplitude)
...
ggplot(FS_by_subj %>% filter(ROI_delateralized=="Putamen"),
       aes(x=mean_amplitude,y=mean_post_pre_rt_change,
           label=subid,color=mean_trough_offset))+geom_point(binwidth=0.2)+geom_text(size=3,nudge_x=0.1)
...
> Post-error slowing significantly (α < 0.05, corrected) deactivated the predicted structures, which were used to generate ROI estimates of activity changes across the other phases of the task
