---
title: "SST Paper generation Wave 1 Only"
author: "Ben Smith"
date: "2022-12-19"
output: 
  html_document: 
...
output_folder <- "sst_paper_generation_w1_no_prop_correct_threshold_output"
...
```{r}
cache_filepath <- paste0(dropbbox_dir,"sst_paper_generation_w1_no_prop_correct_threshold.Rdata")
...
{
  time_points_raw <- readr::read_csv(paste0(dropbbox_dir,"/SST_roi_by_time_point.csv")) %>% filter(wave %in% c(1))
...
if(!loaded_from_cache){
  ...
  sst_all_data_raw <- readr::read_csv(sst_all_data_filepath)  %>% filter(waveid %in% c(1))
...
model_full_effects <- lme4::lmer(
   med_post_trial_z ~ trial_n_s+ condition + P_stop_trial_change_z+ post_pre_rt_change_z + (1 +post_pre_rt_change_z  + P_stop_trial_change_z  | subid),
...
model_no_rt<- lme4::lmer(
   med_post_trial_z ~ trial_n_s+ condition + P_stop_trial_change_z + (1 +post_pre_rt_change_z  + P_stop_trial_change_z  | subid),
  trial_neural_behav_roi %>% filter(trial_n>50 & condition %in% c("CorrectStop","FailedStop"))
  )

anova(model_full_effects,model_no_rt)
model_no_p_stop<- lme4::lmer(
   med_post_trial_z ~ trial_n_s+ condition + post_pre_rt_change_z + (1 +post_pre_rt_change_z  + P_stop_trial_change_z  | subid),
  trial_neural_behav_roi %>% filter(trial_n>50 & condition %in% c("CorrectStop","FailedStop"))
  )
...
behavioral_data_w_tone_data <- readr::read_csv(paste0(dropbbox_dir,"/sst_behavioral_data_all_w_class_type_reveal_onset.csv")) %>% filter(waveid %in% c(1))
