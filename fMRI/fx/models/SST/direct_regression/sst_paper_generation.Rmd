...
```{r}
cache_filepath <- paste0(dropbbox_dir,"sst_paper_generation.Rdata")
...
```{r}
if (!loaded_from_cache){
  save.image(paste0(dropbbox_dir,"/sst_paper_generation.Rdata"))
}
...
```{r }
gtsave(table_cells,filename = "table1.html")
```
...
```
```{r }
gtsave(table_cells,filename = "table2.html")
```
...
```{r }
gtsave(table_cells,filename = "table3.html")
```
