...
motion_prep_load <- function(motion_filename = 'bold_confounds.tsv'){
  confond_pattern <- paste(subPattern, wavePattern, taskPattern, runPattern, motion_filename, sep = "_")
  ...
  for (file_i in 1:length(fileList)) {
    ...
    if (!exists('dataset')) {
      filePattern = paste(subPattern, wavePattern, taskPattern, runPattern, motion_filename, sep = "_")
...
motion_classify_summarize_write <- function(dataset){
  #------------------------------------------------------
  # apply classifier
  #------------------------------------------------------
  # load classifier
